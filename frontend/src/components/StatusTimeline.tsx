import type { TimelineStep } from "@/data/accountOrders";

type StatusTimelineProps = {
  steps: TimelineStep[];
  disabled?: boolean;
};

export default function StatusTimeline({ steps, disabled = false }: StatusTimelineProps) {
  return (
    <ol
      className={`relative mt-6 space-y-0 ${disabled ? "opacity-50" : ""}`}
      aria-disabled={disabled || undefined}
    >
      {steps.map((step, index) => {
        const isLast = index === steps.length - 1;
        const completed = disabled ? false : step.completed;
        const current = disabled ? false : step.current;

        return (
          <li key={step.id} className="relative flex gap-4 pb-8 last:pb-0">
            {!isLast ? (
              <span
                className={`absolute left-[7px] top-4 h-[calc(100%-4px)] w-px ${
                  completed ? "bg-emerald-400" : "bg-zinc-200"
                }`}
                aria-hidden="true"
              />
            ) : null}
            <span
              className={`relative z-10 mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full border ${
                completed
                  ? "border-emerald-600 bg-emerald-600"
                  : current
                    ? "border-emerald-600 bg-white"
                    : "border-zinc-200 bg-white"
              }`}
              aria-hidden="true"
            >
              {completed ? (
                <span className="block h-1.5 w-1.5 rounded-full bg-white" />
              ) : current ? (
                <span className="block h-1.5 w-1.5 rounded-full bg-emerald-600" />
              ) : null}
            </span>
            <div className="min-w-0 flex-1 pt-0">
              <p
                className={`text-xs font-light uppercase tracking-[0.12em] ${
                  disabled ? "text-zinc-400" : "text-zinc-900"
                }`}
              >
                {step.label}
              </p>
              {step.date ? (
                <p
                  className={`mt-1 text-[11px] font-light ${
                    disabled ? "text-zinc-400" : "text-zinc-900"
                  }`}
                >
                  {step.date}
                </p>
              ) : null}
            </div>
          </li>
        );
      })}
    </ol>
  );
}
