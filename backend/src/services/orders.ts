...
import {
  shouldSyncShiprocketStatus,
  syncShiprocketToOrder,
} from "./shiprocketSync.js";
...
async function syncShiprocketForCustomerOrders(
  orders: Array<{
    id: string;
    shiprocketShipmentId: number | null;
    status: import("../generated/prisma/client.js").OrderStatus;
  }>,
) {
  const toSync = orders.filter(
    (order) => order.shiprocketShipmentId && shouldSyncShiprocketStatus(order.status),
  );

  await Promise.all(
    toSync.map((order) =>
      syncShiprocketToOrder(order.id).catch((error) => {
        console.warn(`Shiprocket sync for customer order ${order.id}:`, error);
      }),
    ),
  );
}
...
export async function getOrdersForUserId(userId: string): Promise<AccountOrderDto[]> {
  ...
  await syncShiprocketForCustomerOrders(orders);
  ...
  return refreshed.map(mapOrderToDto);
}
...
export async function getOrderByIdForUserId(
  ...
  if (order.shiprocketShipmentId && shouldSyncShiprocketStatus(order.status)) {
    try {
      ...
      console.warn(`Shiprocket sync for customer order ${orderId}:`, error);
    }
  }
